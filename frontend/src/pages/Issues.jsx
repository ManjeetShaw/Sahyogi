...
export default function Issues() {
  ...
  function load() {
    ...
    api
      ...
      .catch((err) => {
        setIssues([]);
        setError(
          err.response?.data?.message || "Couldn't load issues right now. Please try again."
        );
      })
  ...
  async function handleStatusChange(id, newStatus) {
    try {
      ...
      setError(err.response?.data?.message || "Couldn't update that issue's status.");
    }
  }
  ...
  async function handleDelete(id) {
    try {
      ...
      setError(err.response?.data?.message || "Couldn't delete that issue.");
    }
  }
  ...
  return (
    <div className="container">
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <h3>Couldn&apos;t load issues</h3>
          <p>{error}</p>
          <button className="btn btn-outline" onClick={load} style={{ marginTop: 8 }}>
            Retry
          </button>
        </div>
      ) : issues.length === 0 ? (
